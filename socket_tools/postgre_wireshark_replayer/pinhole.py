"""
taken from: http://code.activestate.com/recipes/114642-pinhole/
with some small modifications by carlos
    ...
    def run( self ):
        while 1:
            try:
                ...
                if not data: break
                self.sink.send( data )
        	log("\nSending {} bytes: {}\n".format(len(data), repr(data)))
        	log("\nHex: {}\n".format(str_to_hex(data)))
		b, msglen = struct.unpack("!ci", data[0:5])
		if(b=="Q"):
                	log("tipo de peticion =%s"%b)
                	query="Received query:{}".format(repr(data[5:]))
                	#log(re.sub(r'[^\w "\';:]', '', query))
                	log(query))
            except:
                break
...
if __name__ == '__main__':
    ...
    if len( sys.argv ) > 1:
        ...
        Pinhole( 5432, 'localhost', 9876).start() #postgresql
        Pinhole( 8000, 'localhost', 80).start()
