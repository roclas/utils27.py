...
Modified by Carlos Hernandez ( who downloaded the original from http://code.activestate.com/recipes/498249-composite-design-pattern-using-dictionaries/history/2/ )
"""
...
if __name__=="__main__":
    p = Properties()
    p.load(open( os.path.dirname(os.path.realpath(__file__))+ '/rgd.properties'))
    p.list()
    print p
    print p.items()
    print p["dest_dir_data"]
    print p["dest_dir_dates"]
    print p["origin_dir"]
    #p['name3'] = 'changed = value'
    #p['new key'] = 'new value'
    p.store(open('rgd.properties','w'))
